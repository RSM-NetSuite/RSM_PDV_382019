...
define(['N/record', 'N/render', 'N/config', 'N/runtime', 'N/file', 'N/query', 'N/url', 'N/email', 'N/log', 'N/search', './dateUtil.js', 'N/redirect'],
  function (record, render, config, runtime, file, query, url, email, log, search, dateUtil, redirect) {
    ...
    function post(requestBody) {
      ...
      if (requestBody.action === 'createbillcreditio') {
        try {
          try {
            //Load bill record
            var billCreditRecord = record.load({
          ...
          try {
            ...
            var vendorId = billCreditRecord.getValue({
              fieldId: 'entity'
            });
            ...
            var currencyRec = record.load({
              ...
              id: billCreditRecord.getValue('currency')
            });
          ...
          var itemLineCount = billCreditRecord.getLineCount({
            sublistId: 'item'
          });

          var expenseLineCount = billCreditRecord.getLineCount({
            sublistId: 'expense'
          });
          ...
          function getItems(itemLineCount) {
            ...
            for (var i = 0; i < itemLineCount; i++) {
              var currentTaxCodeId = billCreditRecord.getSublistValue({
                sublistId: 'item',
                fieldId: 'taxcode',
                line: i
              });
              ...
              if (taxCodeLookup.isreversecharge) {
                ...
                var amt = parseFloat(billCreditRecord.getSublistValue({
                  sublistId: 'item',
                  fieldId: 'amount',
                  line: i
                }));
                var quantity = billCreditRecord.getSublistValue({
                  sublistId: 'item',
                  fieldId: 'quantity',
                  line: i
                })
                ...
                if (billCurrency !== 'RSD') {
                  var exchangeRate = billCreditRecord.getValue('exchangerate');
                  amt *= exchangeRate;
                  taxAmt *= exchangeRate;
                ...
                var name = billCreditRecord.getSublistText({
                  sublistId: 'item',
                  fieldId: 'item',
                  line: i
                });
                ...
                items.push({
                  name: name,
                  description: billCreditRecord.getSublistValue({
                    sublistId: 'item',
                    fieldId: 'description',
                    line: i
                  }),
                  units: billCreditRecord.getSublistText({
                    sublistId: 'item',
                    fieldId: 'units',
                    line: i
                  }),
          ...
          if (subsidiaryFeatureCheck) {
            ...
            var subsidiaryId = billCreditRecord.getValue({
              fieldId: 'subsidiary'
            });
          ...
          try {
            datumIzdavanja = billCreditRecord.getValue({
          ...
          data.tranid = billCreditRecord.getValue('tranid');
          data.tranDate = dUtil.getDateFromFormattedDate(dUtil.formatDate(billCreditRecord.getValue('trandate')));
          data.dueDate = dUtil.getDateFromFormattedDate(dUtil.formatDate(billCreditRecord.getValue('duedate')));
          data.datumIzdavanjaFakture = dUtil.getDateFromFormattedDate(dUtil.formatDate(datumIzdavanja));

          data.brojModela = billCreditRecord.getValue('custbody_broj_modela');
          data.pozivNaBroj = billCreditRecord.getValue('custbody_poziv_na_broj');

          data.memo = billCreditRecord.getValue('memo');
          data.ioTransactionNumber = billCreditRecord.getValue('custbody_rsm_io_counter');

          data.currency = billCreditRecord.getText('currency');
          ...
          var bill_credit_io_pdf_rs = configRecord.getValue({
            fieldId: 'custrecord_rsm_config_io_pdf'
          });

          renderer.setTemplateByScriptId(bill_credit_io_pdf_rs)

          var pdfFile = renderer.renderAsPdf();

          // Delete the old pdf file if it already exists
          var oldFileId = billCreditRecord.getValue('custbody_cust_dep_pdf_file');
          if (oldFileId) {
            file.delete({
              id: oldFileId
            });
          ...
          var newPdfFile = file.create({
            name: "PDF Interni Obracun - bill credit:" + requestBody.transactionId,
          ...
          message = {
            type: 'confirmation',
            titple: 'Uspesno!',
            message: "PDF Interni obracun za bill credit" + requestBody.transactionId + " je uspesno kreiran! Osvezite stranicu",
            duration: '0'
          }
        } catch (error) {
          message = {
